...
class Parameter:
    ...
    def from_rcl_interface_parameter(cls, rcl_param):
        ...
        if Parameter.Type.BOOL == type_:
            value = rcl_param.value.bool_value = obj.value
    ...
    def __init__(self, name, type_, value):
        if not isinstance(type_, Parameter.Type):
            raise RuntimeError("type must be a member of Parameter.Type enum")
    ...
    def get_descriptor(self):
        return ParameterDescriptor(name=self.name, type=self._type.value)
